...
| 特性                    | 说明                                                                                             |
| ----------------------- | ------------------------------------------------------------------------------------------------ |
| **🎯 并行会话**         | 每个会话状态完全隔离：独立的发送状态、流订阅、消息缓存、输入草稿。会话 A 写代码时切换到 B 提问，A 的回复继续接收不受影响 |
| **📦 消息即时存盘**     | 发消息即写入本地 SQLite 数据库，崩溃不丢。AI 回复流完成后自动持久化，关闭程序也不丢消息        |
| **🔄 流式并行**         | 每个会话独立的 SSE 流订阅，切换会话不打断后台接收。消息流结束时直接持久化到 DB，不依赖内存暂存  |
| **🔗 会话续聊**         | Gateway session ID 存本地数据库，重启应用后选中会话继续聊，上下文不丢失                          |
| **🔗 展示层隔离**       | DisplaySession 架构：本地 display_id 驱动列表，切换模型只更新 backend 映射，title/remark 不丢失 |
...
│  │              Local Storage Layer                     │   │
│  │  ┌──────────────────┐  ┌──────────────────────┐    │   │
│  │  │ desktop_config   │  │ desktop_db.sqlite     │    │   │
│  │  │ .json            │  │ (会话 + 消息 + 预览)  │    │   │
│  │  └──────────────────┘  └──────────────────────┘    │   │
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
...
- **并行会话**：每个会话状态完全隔离，切换会话不中断另一个会话的 AI 回复
- **流式输出**：SSE（Server-Sent Events）实时显示 AI 回复，逐 token 展示
- **消息即时存盘**：用户消息发送前即写入 SQLite 数据库，AI 回复流结束时自动持久化，关闭程序不丢消息
...
从 [Releases 页面](https://github.com/lovesmile/hermes-desktop-ui/releases) 下载 `HermesDesktop-Setup-v1.0.8.exe`，
双击安装，打开即用。内置 Inno Setup 安装器，自动注册桌面快捷方式和环境变量。
...
| `~/.hermes/desktop_db.sqlite`   | 本地 SQLite 数据库（所有会话和消息）                    |
...
`desktop_db.sqlite` 由客户端自行管理（SQLite via `sqflite_common_ffi`），与 Hermes 服务端无关。支持双隔离层：

- **连接模式隔离**：local/embedded/remote 各用独立 DB 文件（`desktop_db.sqlite`, `desktop_db_embedded.sqlite`, `desktop_db_<ip>.sqlite`）
- **展示层隔离**：`display_sessions`（UI 展示条目）↔ `sessions`（后端会话数据）

### 表结构

- **`sessions`** — 后端层，以 Gateway session_id 为 key，存储原始消息
- **`messages`** — 消息表，`session_id` 外键关联 sessions
- **`display_sessions`** — 展示层，每个条目对应一个用户聊天。`current_backend_id` 绑定后端 session，切换模型时只更新此字段，title/remark 不丢失

### 自动迁移

v1.0.3 及以前的老 `desktop_db.json` 首次启动自动迁移至 SQLite，JSON 文件重命名为 `.migrated` 备份。
...
2. 复制 VC++ CRT 运行时 DLL
3. PyInstaller 打包 hermes.exe
4. Inno Setup 编译安装包 `HermesDesktop-Setup-v1.0.8.exe`
5. 上传安装包 + hermes-bundle 到 GitHub Release
...
| 本地存储 | SQLite（`desktop_db.sqlite`）+ JSON 配置文件（`desktop_config.json`） |
