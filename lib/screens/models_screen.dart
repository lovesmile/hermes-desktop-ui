...
class _ModelsScreenState extends State<ModelsScreen> {
  ...
  Future<void> _confirmDeleteSkill(String path, String name) async {
    ...
    try {
      await HermesFileService().deleteDir(path);
      HermesFileService.clearCache();
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('删除「$name」失败: $e'), backgroundColor: AppTheme.error),
        );
      }
    }
  }
      ...
      body: _loading
          ...
          : Column(
              children: [
                ...
                Expanded(child: _buildInstalledTab()),
              ],
            ),
    );
  }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Icon(Icons.memory_outlined, color: AppTheme.primary, size: 20),
              ...
              if (_modelConfig.isEmpty)
                ...
                ..._modelConfig.entries.map((e) => _configRow(e.key, e.value)),
            ...
            child: InkWell(
              ...
              onSecondaryTapUp: (details) {
                showMenu<String>(
                  ...
                  items: [
                    PopupMenuItem(value: 'invoke', child: Row(children: [
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: Icon(Icons.auto_awesome, size: 18, color: AppTheme.secondary),
                    ),
                    ...
                    IconButton(
                      ...
                      tooltip: '删除技能',
                      onPressed: () => _confirmDeleteSkill(path, name),
                    ),
                  ],
                ),
              ),
            ...
            content: SizedBox(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Provider', style: TextStyle(fontSize: 13,
                        color: Theme.of(context).colorScheme.onSurfaceVariant)),
                    ...
                    const SizedBox(height: 12),
                    Text('模型', style: TextStyle(fontSize: 13,
                        color: Theme.of(context).colorScheme.onSurfaceVariant)),
                    const SizedBox(height: 6),
                    ...
                    const SizedBox(height: 12),
                    Text('Base URL', style: TextStyle(fontSize: 13,
                        color: Theme.of(context).colorScheme.onSurfaceVariant)),
                    const SizedBox(height: 6),
                    TextField(
                    ...
                    const SizedBox(height: 12),
                    Text('API Key（留空则不修改）', style: TextStyle(fontSize: 13,
                        color: Theme.of(context).colorScheme.onSurfaceVariant)),
                    const SizedBox(height: 6),
                    TextField(
      ...
      if (apiKey.isNotEmpty) {
        ...
        if (envContent.contains('$keyVar=')) {
          envContent = envContent.replaceAll(
            RegExp('^$keyVar=.*' r'$', multiLine: true), '$keyVar=$apiKey');
        } else {
          envContent += '\n$keyVar=$apiKey\n';
        }
        if (baseUrl.isNotEmpty) {
          ...
          if (envContent.contains('$urlVar=')) {
            envContent = envContent.replaceAll(
              RegExp('^$urlVar=.*' r'$', multiLine: true), '$urlVar=$baseUrl');
          } else {
            envContent += '\n$urlVar=$baseUrl\n';
          }
        }
  ...
  Widget _configRow(String key, String value) {
    return Padding(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          SizedBox(width: 100, child: Text(key, style: TextStyle(
            fontSize: 13,
            color: Theme.of(context).colorScheme.onSurfaceVariant,
          ))),
